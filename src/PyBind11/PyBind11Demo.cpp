...
pybind11::array derived_func(long gid, const char* fname) {
    ...
    auto array = pybind11::array_t<double>(
        {grid_dim[2], grid_dim[1], grid_dim[0]},
        {sizeof(double), sizeof(double) * grid_dim[1], sizeof(double) * grid_dim[1] * grid_dim[2]});
...
pybind11::dict get_remote_field(const pybind11::list& py_fname_list, const pybind11::list& py_to_prepare_list,
                                const pybind11::list& py_nonlocal_id_list,
                                const pybind11::list& py_nonlocal_rank_list) {
#ifdef SUPPORT_TIMER
    SET_TIMER(__PRETTY_FUNCTION__, &timer_control);
#endif
    std::cout << "[PyBind11] get_remote_field" << std::endl;

    for (auto item : py_fname_list) {
        std::cout << item.cast<std::string>() << std::endl;
    }

    for (auto item : py_to_prepare_list) {
        std::cout << item.cast<long>() << std::endl;
    }

    for (auto item : py_nonlocal_id_list) {
        std::cout << item.cast<long>() << std::endl;
    }

    for (auto item : py_nonlocal_rank_list) {
        std::cout << item.cast<int>() << std::endl;
    }

    pybind11::dict remote_field;
    pybind11::array_t<double> array =
        pybind11::array_t<double>({2, 2, 2}, {sizeof(double), sizeof(double) * 2, sizeof(double) * 2 * 2});
    remote_field[pybind11::int_(1)] = pybind11::dict();
    remote_field[pybind11::int_(1)]["CCTwos"] = array;
    double* data_ptr = array.mutable_data();
...
int PyBind11SetParameters(struct yt_param_yt* yt_param_ptr) {
    ...
    param_yt["domain_dimensions"] = pybind11::make_tuple(
        yt_param_ptr->domain_dimensions[0], yt_param_ptr->domain_dimensions[1], yt_param_ptr->domain_dimensions[2]);
    param_yt["domain_left_edge"] = pybind11::make_tuple(
        yt_param_ptr->domain_left_edge[0], yt_param_ptr->domain_left_edge[1], yt_param_ptr->domain_left_edge[2]);
    param_yt["domain_right_edge"] = pybind11::make_tuple(
        yt_param_ptr->domain_right_edge[0], yt_param_ptr->domain_right_edge[1], yt_param_ptr->domain_right_edge[2]);
    param_yt["periodicity"] =
        pybind11::make_tuple(yt_param_ptr->periodicity[0], yt_param_ptr->periodicity[1], yt_param_ptr->periodicity[2]);
...
int PyBind11SetFields(struct yt_field* yt_field_ptr, int len) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        field_list[yt_field_ptr[i].field_name]["attribute"] =
            pybind11::make_tuple(yt_field_ptr[i].field_unit, pybind11::list(), yt_field_ptr[i].field_display_name);
        field_list[yt_field_ptr[i].field_name]["field_type"] = yt_field_ptr[i].field_type;
        field_list[yt_field_ptr[i].field_name]["ghost_cell"] =
            pybind11::make_tuple(yt_field_ptr[i].field_ghost_cell[0], yt_field_ptr[i].field_ghost_cell[1],
                                 yt_field_ptr[i].field_ghost_cell[2], yt_field_ptr[i].field_ghost_cell[3],
                                 yt_field_ptr[i].field_ghost_cell[4], yt_field_ptr[i].field_ghost_cell[5]);
...
int PyBind11InitHier(int num_grids, struct yt_grid** grids_local) {
    ...
    // bind to hierarchy
    hier["grid_left_edge"] = pybind11::memoryview::from_buffer(grid_left_edge,  // buffer pointer
                                                               {num_grids, 3},  // shape (rows, cols)
                                                               {sizeof(double) * 3, sizeof(double)}  // strides in bytes
    );
    hier["grid_right_edge"] =
        pybind11::memoryview::from_buffer(grid_right_edge, {num_grids, 3}, {sizeof(double) * 3, sizeof(double)});
    hier["grid_dimensions"] =
        pybind11::memoryview::from_buffer(grid_dimensions, {num_grids, 3}, {sizeof(int) * 3, sizeof(int)});
    hier["grid_levels"] = pybind11::memoryview::from_buffer(grid_levels, {num_grids, 1}, {sizeof(int), sizeof(int)});
    hier["grid_parent_id"] =
        pybind11::memoryview::from_buffer(grid_parent_id, {num_grids, 1}, {sizeof(long), sizeof(long)});
    hier["proc_num"] = pybind11::memoryview::from_buffer(proc_num, {num_grids, 1}, {sizeof(int), sizeof(int)});
...
#else
                // Let's pass in raw primitive type data pointers for now.
                npy_intp grid_dims[3] = {grid_dimensions[g * 3 + 0], grid_dimensions[g * 3 + 1],
...
int PyBind11CallTestScript() {
    ...
    demo["array"] = pybind11::memoryview::from_buffer(buffer,                                 // buffer pointer
                                                      {2, 4},                                 // shape (rows, cols)
    ...
    demo["memoryview"] = pybind11::memoryview::from_buffer(array,         // buffer pointer
                                                           {10},          // shape
...
int PyBind11CallNumPyTestScript() {
    ...
    //    auto numpy_array_bind = pybind11::array_t<double>({100000, 1}, {sizeof(double), sizeof(double)},
    //    double_array); auto numpy_array_bind_view = numpy_array_bind.mutable_data(); demo["numpy_array_bind"] =
    //    numpy_array_bind;
    //
    ...
    //
    //    pybind11::exec("print(pybind11_libyt.demo['numpy_array_bind'][0, 0])"); // this still prints 0.1 even though
    //    it is changed.
    //
    //    std::cout << "[FLAG] double_array ptr = " << static_cast<void*>(double_array) << std::endl;
    //    std::cout << "[FLAG] get from pybind11 = " << static_cast<void*>(numpy_array_bind.mutable_data()) <<
    //    std::endl;
    //
